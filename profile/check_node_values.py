        ...
        if node.tag == 'sts':
            ...
            for status in node:
                ...
                if 'editor' not in status.attrib:
                    print("WARNING: node status",
                          status.attrib['id'], "has no editor defined.")
                    print(
                        "\t{'driver': '%s', 'value': 0, 'uom': --}," % (status.attrib['id']))
                else:
                    print("\t{'driver': '%s', 'value': 0, 'uom': %s}," % (
                        status.attrib['id'], editors[status.attrib['editor']]))
            print("\t]")
